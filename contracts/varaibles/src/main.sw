...
use std::{asset::*, call_frames::msg_asset_id, constants::DEFAULT_SUB_ID, context::*, token::transfer};
use ::errors::{InitError,AccessControlError, DepositError}
...
impl NativeAsset for Contract {
    ...
    fn deposit() {
        require(storage.owner.is_some(), InitError::NotInitialize)
    ...
    fn send(to:Identity, amount:u64) {
        let sender = msg_sender().unwrap;
        require(sender == storage.owner.unwrap(),AccessControlError::UnauthorizedError);
